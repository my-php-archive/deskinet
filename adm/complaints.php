  ...
  if(!mysql_num_rows($query)) { echo 'No hay posts en revisi&oacute;n en este momento'; } else {
	...
	echo '<script type="text/javascript">var post_c_n = new Array();</script>';
  	for($i=0;$i<$c;$i++) {
		...
		echo '<div id="post_d_'.$qpost['id'].'"><span onmouseover="this.style.backgroundColor = \'transparent\';" class="categoriaPost '.$cat['urlname'].'" alt="'.$cat['name'].'" title="'.$cat['name'].'"></span> ('.gmdate('d/m/Y H:i', $qpost['time']).') <strong><a href="/posts/'.$cat['urlname'].'/'.$qpost['id'].'/'.url($qpost['title']).'.html" target="_blank">'.htmlspecialchars($qpost['title']).'</a></strong> por <a href="/perfil/'.$author['nick'].'/" target="_blank">'.$author['nick'].'</a><br /><br /><a class="btnActions" style="border:1px solid #0C0;background-color:#6FD966;" href="#" onclick="admin_complaints_action(\'accept\', '.$qpost['id'].');return false;">Aceptar</a><a class="btnActions" href="#" onclick="admin_complaints_action(\'validate\', '.$qpost['id'].');return false;">Validar</a><a class="btnActions" style="border:1px solid #C00;background-color:#DC535C;" href="#" onclick="admin_complaints_action(\'reject\', '.$qpost['id'].');return false;">Rechazar</a><br /><br /><div style="clear:both"></div>';
