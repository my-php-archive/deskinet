<?php
if($_GET['ajax']) { include('../config.php'); include('../functions.php'); }
if(!isLogged()) { die; }
...
$pag = (!$_GET['p'] || !is_int($_GET['p']) || $_GET['p'] < 1 ? 1 : mysql_clean($_GET['p']));
	...
	$display .= '<li class="resultBox">
			...
			<div class="floatL infoBox">
				<ul>
					...
					<l title="'.htmlspecialchars($group['description']).'">'.(strlen($group['description']) > 60 ? substr($group['description'], 0, 60).'...' : $group['description']).'</li>
...
if($currentPage > 1) { echo '<div class="before floatL"><a href="#" onclick="mygroups_page((currentPage-1));return false;"><b>&laquo; Anterior</b></a></div>'; }
...
$endI = (($currentPage+6) < $totalPages ? $currentPage+6 : $totalPages);
...
if($currentPage < $totalPages) { $display .= '<div class="next floatR"><a href="#" onclick="mygroups_page((currentPage-1));return false;"><b>Siguiente &raquo;</b></a></div>'; }
$display .= '</div>';
...
if($_GET['ajax']) {
	...
	die(strlen($sep).'.'.$sep.$displayStart.$sep.$displayEnd.$sep.$totalGroups.$sep.$_GET['ob'].$display);
}
} // jab grups
?>