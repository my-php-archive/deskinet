...
function admin_complaints_action(sa, post) {
    ...
    ajax.open("GET", "/ajax/admin-complaints.php?rnd=" + parseInt(Math.random()*99999) + "&sa=" + (sa == 'validate' ? 'accept&validate=1' : sa) + "&id=" + post, true);
	ajax.onreadystatechange = function() {
		if(ajax.readyState == 4) {
        	if(ajax.responseText.substring(0, 1) == '0') { alert(ajax.responseText.substring(1)); return false; }
            $('#post_d_' + post).slideUp('fast');
            $('#post_d_' + post).html('La denuncia ha sido ' + (sa == 'accept' ? 'aceptada' : (sa == 'validate' ? 'validada' : 'rechazada')));
...
function open_login_box() {
	...
	if(document.getElementById('login_box').style.display == 'block') { $('#login_box').fadeOut('fast'); } else { $('#login_box').fadeIn('fast'); $('#nickname').focus(); }
}
...
function login_ajax() {
	...
	$('#login_button').enablebutton(false);
		...
		if(ajax.readyState == 4) {
        	if(ajax.responseText == '1') {
                ...
                if(Math.ceil((comm_tcom+1)/dn) > comm_totalpages && Math.ceil((comm_tcom+1)/dn) != 1) {
                	...
                	ajax.open("GET", "/ajax/" + (group ? 'group' : 'post') + "-comments.php?rnd=" + parseInt(Math.random()*99999) + "&post_id=" + post_id + "&post_private=" + post_private + "&author=" + post_author + "&cpage=1&i=" + (comm_tcom-1) + "&if=1&lc=1" + (group ? "&group=" + group_id : ''), true);
               		ajax.onreadystatechange = function() {
    					if(ajax.readyState == 4) {
                        	if(comm_tcom > 1) {
...
function comments_goto(page, group) {
	if(page < 1 || page > comm_totalpages) { return false; }
    	ajax.open("GET", "/ajax/" + (group ? 'group' : 'post') + "-comments.php?rnd=" + parseInt(Math.random()*99999) + "&post_id=" + post_id + "&post_private=" + post_private + "&author=" + post_author + "&cpage=" + page + "&if=1&i=" + ((parseInt(page)-1)*(group ? 20 : 100)) + "&dh=" + dh.substring(1) + (group ? '&group=' + group_id : ''), true);
	ajax.onreadystatechange = function() {
    		if(ajax.readyState == 4) {
...
function delete_comment(id, group) {
	ajax.open("GET", "/ajax/" + (group ? 'group-' : '') + "delete-comment.php?rnd=" + parseInt(Math.random()*99999) + "&id=" + id, true);
	ajax.onreadystatechange = function() {
		if(ajax.readyState == 4) {
        	if(ajax.responseText == '1') {
...
function np_previsualizar(form, vp){
	...
	ajax.onreadystatechange = function() {
		if(ajax.readyState == 4) {
            ...
            document.getElementById('previewc').style.display = 'inline';
            scrollUp();
        }
	};         
    ajax.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
...
function reg_validate_data(form) {
    ...
    if(!/^[0-9]+$/.test(day) || !/^[0-9]+$/.test(month) || !/^[0-9]+$/.test(year) || day.length > 2 || month.length > 2 || year.length > 4 || day > 31 || month > 12 || year > gyear) { form.day.value = ''; form.month.value = ''; form.year.value = ''; form.day.focus(); alert('Introduce una fecha de nacimiento v\xe1lida'); return false; }
    if((year-min_age) > (gyear-min_age)) { alert('Debes tener al menos ' + min_age + ' a\xf1os'); return false; }
		...
		success: function(h){
			switch(h.charAt(0)){
				case '0': //Error
					...
					break;
				case '1': //OK. Ya es miembro -.- no jodas
					mydialog.body(h.substring(1), 305);
					break;
			}
			mydialog.center();
		},
...
function ec_validate_data(form) {
    ...
    if(!/^[0-9]+$/.test(day) || !/^[0-9]+$/.test(month) || !/^[0-9]+$/.test(year) || day.length > 2 || month.length > 2 || year.length > 4 || day > 31 || month > 12 || year > gyear) { form.dia.value = ''; form.mes.value = ''; form.ano.value = ''; form.dia.focus(); alert('Introduce una fecha de nacimiento v\xe1lida'); return false; }
    if((year-min_age) > (gyear-min_age)) { alert('Debes tener al menos ' + min_age + ' a\xf1os'); return false; }
...
function monitor_pages(n) {
	...
	$.ajax({
        	...
        	data: 't=' + n,
        	dataType: 'html',
        	beforeSend: function() { $('#mpli').css('display', 'block'); },
        	success: function(t) {
        		$('#mpli').css('display', 'none');
...
function poll_r(b) {
	$('#vote').toggle();
    $('#results').toggle();
	$(b).html(($('#vote').css('display')=='block' ? 'Ver resultados' : 'Ver opciones'));
}
	...
	$.ajax({
        ...
        success: function(t) {
            ...
            if(t != '0') {
                ...
                if(!$('div.alertas').length) { $('div.userInfoLogin > ul > li.monitor').append('<div class="alertas"><a><span>' + hmn + '</span></a></div>'); }
            	$('div.notificaciones-list ul').html(text);
                noti_alert_animate();
                clearInterval(noti_alert_time);
                var noti_alert_time = setInterval('noti_alert_animate();', 10000);
            }
		}
	});
}

if(isLogged) { setInterval('noti_update();', 10000); }
...
function TopsTabs(pref, filter) {
    ...
    if(pref == 'posts_') {
    	...
    	if(t_groups[pn] < t_groups[t_groups_selected]) {
        	document.getElementById('box_c_groups').style.height = parseInt(document.getElementById('box_c_groups').offsetHeight+t_groups[pn]+5) + 'px';
        } else {
        	if(filter == 'Semana') {
...
$(document).ready(function(){
    ...
    $('div.notificaciones-list').bind('blur',function(){noti_hidelast();}).hover(function(){$(this).unbind('blur');},function(){$(this).bind('blur',function(){noti_hidelast();});});
