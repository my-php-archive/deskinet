$(document).ready(function(){
	$('textarea.imagen-desc').live('focus', function(){if($(this).html()=='Descripci&oacute;n de la foto')$(this).html('');}).live('blur', function(){if($(this).html()=='')$(this).html('Descripci&oacute;n de la foto');});
	$('input[name=ciudad]').autocomplete('/ajax/locations.php', {
		minChars: 2,
		width: 298
...
function input_fake(x) {
	$('.input-hide-'+x).hide();
	$('input.input-hidden-'+x).show().focus();
}
...
var cuenta = {
	...
	save: function(secc, next) {
		...
		$.ajax({
            ...
            data: params.join('&'),
            dataType: 'json',
            error: function(x,e){alert(x.responseText+'+'+e);},
			success: function(r) {
				if (r.error) {
					...
					cuenta.alert(secc, 'Los cambios han sido aceptados y aplicados');
	...
	imagen: {
		...
		add: function(obj) {
			...
			$.ajax({
				type: 'post',
                url: '/ajax/account.php',
                data: 'save=7&action=add&url='+encodeURIComponent($(url).val())+'&caption='+encodeURIComponent($(caption).val()),
                dataType: 'json',
				success: function(r) {
					if (r.error) {
						...
						$(obj).attr('onclick', '').unbind('click').bind('click', function(){ cuenta.imagen.del(this, r.id); }).removeClass('misfotos-add').addClass('misfotos-del').html('Eliminar');
						$(url).parent().parent().prepend('<div class="floatL"><img src="'+$(url).val()+'" class="imagen-preview" /></div>')
						$('<div class="field"><label>Imagen</label><div class="input-fake"><div class="floatL"><input style="width:300px;" value="http://" type="text" class="text" /><textarea class="imagen-desc" style="margin-top:5px;width:300px">Descripci&oacute;n de la foto</textarea><a onclick="cuenta.imagen.add(this)" class="misfotos-add">Agregar</a></div><div class="clearfix clearBoth"></div></div></div>').appendTo('.content-tabs.mis-fotos > fieldset');
					}
				}
			});
		},
...
var avatar = {
	...
	upload: function(obj) {
		if ($(obj).hasClass("local")) {
			if ($('input#file-avatar').val()) {
				...
				$.ajaxFileUpload({ url: '/ajax/avatar-upload.php', fileElementId: 'file-avatar', dataType: 'json', success: this.uploadsuccess });
			}
	...
	crop: function(img) {
		...
		$('img.avatar-big').attr('src', img+'?'+Math.random()).bind('load', function(){ $('img.avatar-crop').Jcrop({ aspectRatio: 1, sideHandles: false, setSelect: [ 0, 0, 50, 50 ], onChange: avatar.preview, onSelect: function(c) { avatar.crop_coord = c; } }) });
	},
	reload: function() {
	...
	save: function() {
		...
		else $.get('/ajax/avatar-upload.php?save=1&coords='+this.crop_coord.x+','+this.crop_coord.y+','+this.crop_coord.x2+','+this.crop_coord.y2,function(r){if(r.error)alert(r.error);else mydialog.close();});
	},
...
jQuery.extend({
	...
	createUploadForm: function(id, fileElementId) {
		...
		var form = $('<form	action="" method="POST" name="' + formId + '" id="' + formId + '" enctype="multipart/form-data"><input type="hidden" name="maxw" value="'+$(window).width()+'" /><input type="hidden" name="maxh" value="'+$(window).height()+'" /></form>');
