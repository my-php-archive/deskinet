...
var registro = {
    ...
    banned_emails: ['@yopmail.com','@yopmail.fr','@yopmail.net','@jetable.fr.nf','@nospam.ze.tc','@nomail.xl.cx','@mega.zik.dj','@speed.1s.fr','@cool.fr.nf','@courriel.fr.nf','@moncourrier.fr.nf','@monemail.fr.nf','@monmail.fr.nf'],
    dialog: true,
	paso_actual: 1,
	datos: new Array(),
		...
		switch(campo){
			...
			case 'nick':
				...
				if(value.length<6 || value.length>15)
					return this.show_status(el, 'error', 'Debe tener entre 6 y 15 caracteres');
...
//					return this.show_status($, 'error', 'La contrase&ntilde;a tiene que ser distinta que el nick');
				...
				$.ajax({
					...
					success: function(h){
						...
						registro.cache[campo][value_lower]['text'] = h.substring(1);
						switch(h.charAt(0)){
							case '0': //Estaba en uso
								registro.cache[campo][value_lower]['status'] = false;
								registro.show_status(el, 'error', h.substring(1));
								break;
			...
			case 'password':
				...
				if(value.length<6 || value.length>35)
					return this.show_status(el, 'error', 'Debe tener entre 6 y 35 caracteres');
			...
			case 'email':
                ...
                for(i=0;i<this.banned_emails.length;i++) {
                  if(strpos(value, this.banned_emails[i])) {
                    return this.show_status(el, 'error', 'No admitimos emails ' + this.banned_emails[i]);
                  }
                }
				...
				$.ajax({
					...
					success: function(h){
						...
						registro.cache[campo][value]['text'] = h.substring(1);
						switch(h.charAt(0)){
							case '0': //Estaba en uso
			...
			case 'anio':
				...
				if(!empty(this.datos['dia']) && !empty(this.datos['mes']) && !empty(this.datos['anio'])){
					...
					if(edad(this.datos['mes'], this.datos['dia'], this.datos['anio']) < min_age)
						return this.show_status(el, 'error', 'Debes tener al menos ' + min_age + ' a&ntilde;os');
			...
			case 'pais':
				...
				$.ajax({
					...
					data: 'type=provinces&country=' + value,
					success: function(h){
						switch(h.charAt(0)){
							case '0': //Error
								registro.show_status(el, 'error', h.substring(1));
							...
							case '1': //OK
								...
								registro.show_status(el, 'ok', 'OK');
								$('#RegistroForm .pasoDos #provincia').append(h.substring(1)).removeAttr('disabled').val('').focus();
								break;
							case '2': //El pais no tiene provincias
								...
								$('#RegistroForm .pasoDos #ciudad').setOptions({
									extraParams: {'type':'cities', 'country':registro.datos['pais']}
								}).flushCache();
			...
			case 'provincia':
				...
				$.ajax({
					type: 'GET',
					url: '/ajax/locations.php',
					data: 'type=hasCities&province=' + value,
					success: function(h){
						switch(h.charAt(0)){
							case '0': //Error
								registro.show_status(el, 'error', h.substring(1));
							...
							case '1': //OK
								...
								//Autocomplete para las ciudades
								$('#RegistroForm .pasoDos #ciudad').setOptions({
									extraParams: {'type':'cities', 'province':registro.datos['provincia']}
								}).flushCache();
			...
			case 'ciudad':
				...
				$.ajax({
					type: 'GET',
					url: '/ajax/locations.php',
					data: 'type=check&province=' + this.datos['provincia'] + '&city=' + this.datos[campo+'_id'],
					success: function(h){
						switch(h.charAt(0)){
							case '0': //Error
								registro.show_status(el, 'error', h.substring(1));
		...
		$.ajax({
			...
			success: function(h){
				...
				switch(h.substring(0, strpos(h, ':'))){
					...
					case '1':
						if(registro.dialog){
							mydialog.body(h.substring(strpos(h, ':')+1));
							mydialog.buttons(true, true, 'Aceptar', 'mydialog.close()', true, true);
							mydialog.center();
						}else{
							$('.reg-login .registro #RegistroForm').html(h.substring(strpos(h, ':')+1));
