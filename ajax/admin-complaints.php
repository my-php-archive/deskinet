...
if($_GET['sa'] == 'delete') {
	...
	if(!mysql_num_rows($p = mysql_query("SELECT * FROM `posts` WHERE id = '".mysql_clean($_GET['id'])."'"))) { die('0No existe el post'); }
	...
	if($_GET['sa'] == 'accept') {
		...
		$pmm = ($_GET['validate'] == '1' ? "Hemos considerado que tu denuncia en el post \"".htmlspecialchars($post['title'])."\" es correcta.\n\nPor ayudar a la comunidad, has ganado 3 puntos." : "Hemos considerado que el post que denunciaste (".htmlspecialchars($post['title']).") debe ser borrado por no cumplir el [url=http://".$config['script_url']."/protocolo/]protocolo[/url].\n\nPor ayudar a la comunidad, has ganado 3 puntos.");
		while($comp = mysql_fetch_array($query)) {
			mysql_query("INSERT INTO `pms` (user_to, user_from, title, message, time) VALUES ('".$comp['user']."', '".$currentuser['id']."', 'Denuncia aceptada', '".mysql_clean($pmm)."', '".time()."')") or die('06'.mysql_error());
		...
		if(!$_GET['validate'] || $_GET['validate'] != '1') {
            ...
            if($_GET['draft'] == '1') {
                mysql_query("INSERT INTO `drafts` (user, type, title, cat, content, tags, stick, superstick, comments, private, time, `mod`, reason) VALUES ('".$post['author']."', '2', '".mysql_clean($post['title'])."', '".mysql_clean($post['cat'])."', '".mysql_clean($post['message'])."', '".mysql_clean($post['tags'])."', '".$post['sticky']."', '".$post['superstick']."', '".$post['comments']."', '".$post['private']."', '".time()."', '".$currentuser['id']."', 'Por acumulaciÃ³n de denuncias')") or die(mysql_error());
            }
        }
	} else {
		while($comp = mysql_fetch_array($query)) {
			mysql_query("INSERT INTO `pms` (user_to, user_from, title, message, time) VALUES ('".$comp['user']."', '".$currentuser['id']."', 'Denuncia rechazada', 'Hemos considerado que el post que denunciaste (".mysql_clean(htmlspecialchars($post['title'])).") cumple el [url=http://".$config['script_url']."/protocolo/]protocolo[/url].\n\nDebido a esto, te retiramos 3 puntos.', '".time()."')") or die('08'.mysql_error());
