...
function admin_ban(ac, form) {
	...
	ajax.send("sa=" + ra + "&user=" + form.user.value + "&st=" + st + "&ac=" + ac + "&end=" + end + "&ban=" + form.ban.value + "&reason=" + (ra == 'check' || ac == '2' ? '1' : form.reason.value) + "&all=" + ballq);
	ajax.onreadystatechange = function() {
		if(ajax.readyState == 4) {
        	if(ajax.responseText.substring(0, 1) == '0') {
            	document.getElementById('mensajes_div').className = 'error';
...
if($currentuser['rank'] != 8 && $currentuser['rank'] != 6) {
	echo '}';
} elseif($currentuser['rank'] == 8 || $currentuser['rank'] == 6) {
...
function add_comment(id, message, group) {
    ...
    ajax.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
	ajax.send("message=" + encodeURIComponent(message) + "&id=" + id);
	ajax.onreadystatechange = function() {
		if(ajax.readyState == 4) {
        	if(ajax.responseText == '1') {
...
var dh = document.hack;
function comments_goto(page) {
	if(page < 1 || page > comm_totalpages) { return false; }
    ajax.open("GET", "/ajax/post-comments.php?rnd=" + parseInt(Math.random()*99999) + "&post_id=" + post_id + "&post_private=" + post_private + "&author=" + post_author + "&cpage=" + page + "&if=1&i=" + ((parseInt(page)-1)*100) + "&dh=" + dh, true);
	ajax.onreadystatechange = function() {
    		if(ajax.readyState == 4) {
...
function cite_comment(id, author) {
	var bc = document.getElementById('body_comm');
    	if(bc.value == 'Escribir un comentario') { bc.value = ''; }
    	bc.focus();
	ajax.open("GET", "/ajax/comment.php?rnd=" + parseInt(Math.random()*99999) + "&id=" + id, true);
	ajax.onreadystatechange = function() {
		if(ajax.readyState == 4) {
